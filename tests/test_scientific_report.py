...
def _results() -> dict:
    return {
        ...
        "channel_metadata": {"channel_08": {"physical_unit": "cm", "sensor_type": "wave_height"}},
        "basic_stats": {
            "channel_08": {
                "sample_count": 69120,
                "mean": 0.0,
...
def test_report_marks_unreliable_peak_and_documents_method():
    ...
    assert "Hmin=0.6" in text
    assert "Hmoy=2.4" in text
    assert "EN ATTENTE DE VALIDATION INGÉNIEUR" in text
...
def test_html_report_contains_scientific_sections_and_plot():
    ...
    assert "Statistiques temporelles et hauteurs par voie" in rendered
    assert "Étude spectrale par voie" in rendered
    assert "Hmin" in rendered
    assert "Hmoy" in rendered
    assert "H1/3" in rendered
    assert "Hmax" in rendered
    assert "N = nombre d’échantillons" in rendered
    assert "Décision ingénieur" in rendered
    assert "REJETÉ PAR L’INGÉNIEUR" not in rendered


def test_report_uses_only_explicit_engineer_decisions_for_acceptance():
    results = copy.deepcopy(_results())
    results["quality"]["channel_08"]["engineer_decision"] = "accepted"

    rendered = build_scientific_report_html(results, None)

    assert "VALIDÉ PAR L’INGÉNIEUR" in rendered
    assert "ACCEPTÉ PAR L’INGÉNIEUR" in rendered
